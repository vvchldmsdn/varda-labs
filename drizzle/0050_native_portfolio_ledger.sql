-- Additive opt-in ledger. No reconstruction/backfill of legacy cash or costs.
ALTER TABLE accounts ADD COLUMN native_state jsonb;
ALTER TABLE accounts ADD CONSTRAINT accounts_native_state_check CHECK (native_state IS NULL OR (native_state->>'version' = '1' AND native_state->>'accountId' = id::text AND octet_length(native_state::text) <= 1000000));
ALTER TABLE event_ledger_entries ADD COLUMN native_data jsonb;
ALTER TABLE event_ledger_entries ADD COLUMN native_sequence integer;
ALTER TABLE event_ledger_entries ADD COLUMN native_operation_id uuid;
ALTER TABLE event_ledger_entries ALTER COLUMN legacy_asset_id DROP NOT NULL;
ALTER TABLE event_ledger_entries ADD CONSTRAINT event_ledger_native_check CHECK (
 (native_data IS NULL AND native_sequence IS NULL AND native_operation_id IS NULL AND legacy_asset_id IS NOT NULL)
 OR (native_data IS NOT NULL AND native_sequence >= 0 AND native_operation_id IS NOT NULL AND canonical_owner_user_id IS NOT NULL AND account_id IS NOT NULL AND source = 'native_ledger_v1' AND NOT is_sample));
CREATE UNIQUE INDEX event_native_sequence_unique ON event_ledger_entries(canonical_owner_user_id,account_id,native_sequence) WHERE native_data IS NOT NULL;
CREATE UNIQUE INDEX event_native_operation_unique ON event_ledger_entries(canonical_owner_user_id,account_id,native_operation_id) WHERE native_data IS NOT NULL;
ALTER TABLE daily_portfolio_snapshots ADD COLUMN native_evidence jsonb;
ALTER TABLE daily_portfolio_snapshots ADD CONSTRAINT snapshot_native_evidence_check CHECK (native_evidence IS NULL OR (source = 'native_ledger_v1' AND NOT is_sample AND octet_length(native_evidence::text) <= 2000000));

-- Trusted application writer only, SECURITY INVOKER: no tenant privilege widening.
-- State calculations happen in the common exact engine; this boundary atomically
-- fences stale reads and updates assets, cash, both sides of transfers and evidence.
CREATE FUNCTION apply_native_portfolio_mutation(p_owner uuid, p_operation uuid, p_request jsonb, p_changes jsonb)
...
BEGIN
 ...
 FOR c IN SELECT value FROM jsonb_array_elements(p_changes) LOOP
  ...
  IF c->'event'->>'type' <> 'opening' AND EXISTS (
   ...
   WHERE s.canonical_owner_user_id=p_owner AND s.account_id=a.id AND s.native_evidence IS NOT NULL
    AND (s.native_evidence->'frame'->>'at')::timestamptz >= (c->'event'->>'at')::timestamptz
 ...
 FOR c IN SELECT value FROM jsonb_array_elements(p_changes) LOOP
  ...
  IF c->'newAsset' IS NOT NULL AND c->'newAsset' <> 'null'::jsonb THEN
   ...
   VALUES ((pos->>'id')::uuid,p_owner,a.id,a.code,pos->>'name',pos->>'ticker',pos->>'market',pos->>'currency',pos->>'assetType',0,(entry->>'price')::numeric,'user_native_trade','manual');
  END IF;
