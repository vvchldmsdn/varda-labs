...
const input = { currency: "KRW", rows: [{ name: "My first fund", value: 900000, instrumentId: null }] };
const pg = new PGlite();
let route, queries, resolution, failure;
const request = (method, body, headers = {}) => new Request("https://local.test/api/portfolio-drafts", {
...
describe("isolated approximate portfolio persistence", () => {
  before(async () => {
    ...
    await pg.exec(readFileSync(new URL("../drizzle/0046_portfolio_drafts.sql", import.meta.url), "utf8"));
    ...
    [route, queries] = await importWithPorts(["src/app/api/portfolio-drafts/route.ts", "src/db/queries/portfolio-drafts.ts"], {
  ...
  beforeEach(async () => {
    await pg.exec("truncate portfolio_drafts; update app_users set status = 'active'");
  ...
  it("persists the authenticated owner's immutable plan and makes identical retries idempotent", async () => {
    ...
    const changed = await route.POST(request("POST", { id, input: { ...input, rows: [{ ...input.rows[0], value: 10 }] } }));
    assert.equal(changed.status, 409);
    const list = await route.GET(request("GET"));
    const { drafts } = await list.json();
    assert.equal(drafts.length, 1);
    assert.deepEqual(drafts[0].input, input);
    assert.ok(Number.isFinite(Date.parse(drafts[0].createdAt)));
    assert.equal(list.headers.get("cache-control"), "private, no-store");
    assert.equal(list.headers.get("x-robots-tag"), "noindex, nofollow");
    assert.equal((await pg.query("select relforcerowsecurity from pg_class where relname='portfolio_drafts'")).rows[0].relforcerowsecurity, true);
    assert.deepEqual((await pg.query("select tablename from pg_tables where schemaname='public' order by tablename")).rows.map(r => r.tablename), ["app_users", "investment_plans", "portfolio_drafts"]);
  });
  ...
  it("protects list, delete, insertion and updates across tenants at the database boundary", async () => {
    ...
    assert.deepEqual(await (await route.GET(request("GET"))).json(), { drafts: [] });
    assert.equal((await route.DELETE(request("DELETE", { id }))).status, 404);
    assert.deepEqual(await asTenant("select * from portfolio_drafts", [], other), []);
    await assert.rejects(asTenant("insert into portfolio_drafts(owner_user_id,id,input_json,engine_version) values($1,$2,$3,'amount_composition_v1')", [owner, id, JSON.stringify(input)], other), /row-level security/);
    await assert.rejects(asTenant("update portfolio_drafts set input_json='{}'"), /permission denied/);
    ...
    assert.equal((await pg.query("select count(*)::int as n from portfolio_drafts")).rows[0].n, 2);
  });
  ...
  it("processes overlapping duplicate requests as one creation and one retry", async () => {
    ...
    assert.equal((await pg.query("select count(*)::int as n from portfolio_drafts")).rows[0].n, 1);
  ...
  it("fails closed for stale sessions after app-user deactivation and for missing tenant settings", async () => {
    ...
    assert.equal((await route.DELETE(request("DELETE", { id }))).status, 404);
    assert.deepEqual(await asTenant("select * from portfolio_drafts", [], ""), []);
  });
  ...
  it("enforces 50 plans while retaining idempotent retry at the limit", async () => {
    for (let n = 0; n < 50; n++) {
      ...
      assert.equal((await queries.savePortfolioDraft({ ownerUserId: owner }, planId, input)).status, "created");
    }
    assert.equal((await queries.savePortfolioDraft({ ownerUserId: owner }, "11111111-1111-4111-8111-000000000000", input)).status, "existing");
    assert.deepEqual(await (await route.POST(request("POST", { id, input }))).json(), { error: "draft_limit" });
    assert.equal((await pg.query("select count(*)::int as n from portfolio_drafts")).rows[0].n, 50);
  });
  ...
  it("rejects anonymous/unlinked sessions without saving and accepts retry after authentication", async () => {
    ...
    assert.equal((await pg.query("select count(*)::int as n from portfolio_drafts")).rows[0].n, 0);
  ...
  it("rejects CSRF, oversized bodies, URL inputs, owner injection and malformed plans", async () => {
    const cases = [
      ...
      request("POST", { id, input: { ...input, rows: [{ ...input.rows[0], instrumentId: "spoof" }] } }),
      request("POST", "x".repeat(4097)), request("POST", "{"),
      new Request("https://local.test/api/portfolio-drafts?amount=123", { method: "POST", body: JSON.stringify({ id, input }) }),
    ];
