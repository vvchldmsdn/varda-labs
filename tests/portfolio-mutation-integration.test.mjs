...
const DDL = `
create table accounts(id uuid primary key, canonical_owner_user_id uuid, owner_user_id text, code text, name text, account_type text, currency text, is_active boolean default true, sort_order integer default 0, native_state jsonb, created_at timestamptz default now(), updated_at timestamptz default now(), unique(id,canonical_owner_user_id), unique(canonical_owner_user_id,code));
