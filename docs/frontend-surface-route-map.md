...
| `/investment-lab` | Read-only aggregate counterfactual comparing the observed all-account invested-position path with an all-KODEX-200 path under the same dated KRW buy/sell amounts. Also shows a separately calculated Modified Dietz cashflow-adjusted estimated return when price-basis checks pass. | `src/app/investment-lab/page.tsx`, `getReadOnlyInvestmentLabCounterfactual`, read loader/model, return-estimate composer, pure Modified Dietz helper, existing execution schedule, and deterministic path engine. Reads explicit event, account snapshot, close, and adjusted-close projections in parallel. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read and pure calculation. No browser refetch, provider, mutation, schema, job, or persistence. | Local and production smoke cover auth, path evidence, dynamic aggregate counts, return method/status, leak scan, and unchanged DB counts. Test/lint/build pass. | Aggregate `all` plus KODEX 200 only. Account-specific paths remain blocked by three unattributed events. Exact flow-time TWR, total-return parity, VOO, fixed vectors, scheduled rebalancing, optimization, and transaction costs remain deferred. | User data/meaning review, then choose the next fixed scenario or another primary product slice. |
