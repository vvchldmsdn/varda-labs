...
| `/history` | Read-only balance and portfolio history evidence. Supports account/lane URL filters, explicitly separates balance dates from snapshot dates, labels stored versus display-only derived aggregates, and shows separate stored-amount trajectory charts. Historical values are not recomputed under the current asset policy. | `src/app/history/page.tsx`, `getReadOnlyHistoryBalance`, `history-balance`, `history-trajectory`, and `src/components/history/*`. Reads `account_balance_snapshots` and `daily_portfolio_snapshots`; can derive display-only `all` rows when stored rows are absent. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read and server-rendered SVG. No browser refetch or write. Public DTOs do not select DB UUIDs. | Dedicated local and production smoke covers dashboard/history 401/200, account/lane chart and table states, stored/derived markers, no-interpolation disclosure, full-response ID/secret leakage, overflow ownership, and DB counts unchanged. | Charts connect only consecutive calendar dates with the same source and row kind. Gaps, source changes, and stored/derived changes remain disconnected; there is no interpolation, performance calculation, position drilldown, third history source, or historical rewrite. | Keep missing evidence visible and choose the next product boundary separately. |
