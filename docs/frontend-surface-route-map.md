...
| `/simulation` | Read-only Simulation Validation with a separate owner/account input preflight and the existing retrospective KODEX 200/VOO research surface. The owner section shows current account composition and stored-history readiness without executing a portfolio simulation. The fixed research keeps exact-date 90-step input readiness, bounded carry evidence, observed KRW-return charts, stationary-bootstrap paths, regime-labeled hindsight research, walk-forward minimum-volatility comparison, and seven-endpoint stability history. | `src/app/simulation/page.tsx`, `resolveCurrentTenantContext`, `getReadOnlyTenantSimulationOwnerInputPreflight`, `getReadOnlySimulationInputReadiness`, owner-scoped portfolio structure, the shared stored-history preflight, pure owner-input and matrix/alignment/research modules, and split Simulation Server Components. Owner composition is loaded only through canonical owned accounts; independent fixed-research reads start in parallel after session resolution. | Basic Auth remains the outer gate. The Server Component also requires a resolved Neon Auth session mapped to an active app user and canonical account owner before owner or shared market-evidence reads start. The `account` query is only a validated filter. | Server Component direct DB reads after session resolution and pure in-memory diagnostics/calculations. No browser refetch, provider, mutation, vector resolver, job, or persistence. Fount is removed by the exact approved decision before diagnostic weight derivation; KRX Gold keeps its current weight but requires explicit manual historical evidence. | Pure owner-input fixtures cover aggregation, deterministic 10,000bps diagnostics, explicit 0bps preservation, Fount exclusion, Gold manual-history status, valuation gaps, and account-scope mismatch. Route tests enforce owner-first server reads, account URL preservation, partial diagnostics, and no write/provider boundary. | Owner current-value weights are diagnostics only and are not an admitted scenario vector, target, optimizer input, recommendation, or order authority. Missing valuation blocks vector derivation but remains visible. Missing historical evidence produces partial diagnostics instead of hiding available rows. Existing KODEX/VOO outputs remain fixed retrospective research and must not be labeled as the user's portfolio forecast. | Add a reviewed provider-history completion path and an owner-vector execution contract before rendering owner fan charts. Keep account-specific and all-account execution separate, preserve partial evidence disclosure, and do not silently reuse current weights as approved optimization authority. |
