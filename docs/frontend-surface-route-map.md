...
| `/investment-lab` | Read-only aggregate counterfactual comparing the observed all-account invested-position path with all-KODEX-200 and all-VOO paths under the same dated KRW buy/sell amounts. Shows separately guarded Modified Dietz estimates. | `src/app/investment-lab/page.tsx`, `getReadOnlyInvestmentLabCounterfactual`, read loader/model, return-evidence guard, return-estimate composers, pure Modified Dietz helper, VOO evidence resolver, and deterministic KODEX/VOO path engines. Reads event, account snapshot/cash/FX, KODEX/VOO close, and FX projections in parallel. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read and pure calculation. No browser refetch, provider, mutation, schema, job, or persistence. | Local production-mode smoke with production SELECT data: no-auth 401, auth 200, KODEX and VOO path/return ready, VOO 27/27 valuation close, 27/27 snapshot FX provenance, 38/38 execution FX and applied flows, 5 delayed executions, leak scan pass, DB counts unchanged. | Aggregate `all` only. Account-specific paths remain blocked by three unattributed events. Exact flow-time TWR, total-return parity, fixed vectors, scheduled rebalancing, optimization, and transaction costs remain deferred. | Keep the two fixed scenarios stable and choose the next product slice without widening to runtime vector authority. |
