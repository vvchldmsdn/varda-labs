...
| `/simulation` | Read-only Simulation Validation and retrospective research surface for KODEX 200 and VOO. It keeps exact-date 90-step input readiness, bounded carry evidence, observed KRW-return charts, fixed single/fixed-mix stationary-bootstrap paths, regime-labeled hindsight research, a walk-forward minimum-volatility comparison, and a seven-endpoint stability history for that fixed policy. | `src/app/simulation/page.tsx`, `resolveCurrentTenantContext`, `getReadOnlySimulationInputReadiness`, the batch server-only period preflight, strict query selection, pure matrix/alignment/research modules, and split Simulation Server Components. One bounded price/FX snapshot is read before independent and combined preflights run in memory; the selected walk-forward result and seven exact-endpoint stability rows reuse those combined matrices without another DB read. | Basic Auth remains the outer gate. The Server Component also requires a resolved Neon Auth session mapped to an active app user before starting shared market-evidence reads. | Server Component direct DB read after session resolution and pure in-memory calculations. No browser refetch, provider, mutation, vector resolver, job, or persistence. Empty, whitespace-padded, invalid, and repeated `end` values are blocked before DB reads. | Default smoke verifies no-auth 401 and a fail-closed Basic-auth-only boundary with no product DB read. Session-bound deep smoke requires an explicit cookie and preserves the existing ready/partial/unavailable and unchanged-row-count checks. | All outputs remain fixed retrospective research, not the signed-in user's portfolio, account optimization, prediction, recommendation, or order authority. The walk-forward path uses only the paired KRW-return matrix: rolling 60-row training, the next 10-row test, three folds, long-only fully-invested weights, 10% diagonal covariance shrinkage, costless rebalance at fold boundaries, and no rebalance within a fold. A same-cadence 50:50 path is the neutral comparator. Missing joint evidence marks only the affected row unavailable. Current holdings, targets, Fount, KRX Gold, approved QQQ vector, and durable artifacts do not enter this path. | Add owner/account-bound scenario input as a separate capability; do not silently reinterpret this fixed research model as a user portfolio simulation. Extend other hindsight objectives only with their own no-look-ahead policy. |
