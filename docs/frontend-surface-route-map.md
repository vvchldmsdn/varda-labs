...
| `/simulation` | Read-only Simulation Validation market-input readiness. Checks KODEX 200 and VOO independently for an exact end service date and 90 return steps, and shows a bounded seven-date recheck history. | `src/app/simulation/page.tsx`, `getReadOnlySimulationInputReadiness`, the batch server-only period preflight, strict query selection, pure readiness projection, and `SimulationInputReadinessView`. One bounded price/FX snapshot is read before the independent date/instrument preflights run in memory. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read. No browser refetch, provider, mutation, simulation execution, vector resolver, job, or persistence. Empty, whitespace-padded, invalid, and repeated `end` values are blocked before DB reads. | Pure model, batch-read, query-boundary, and route tests plus dedicated local/production smoke. The smoke accepts only `matrix_ready` or `unavailable`, requires seven bounded history rows, scans for leaks, and compares DB row counts. | Market-data readiness only. The date rows are request-time rechecks, not persisted simulation runs. The page does not expose fan paths, percentiles, initial capital, account selection, target weights, recommendation, or optimization. Exact-endpoint misses are not rolled back automatically; the user can explicitly inspect another displayed date through `?end=YYYY-MM-DD`. | Verify deployed default and explicit-date states, then keep execution runtime blocked until tenant/vector authority and server-owned run parameters are established. |
