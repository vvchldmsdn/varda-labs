...
| `/simulation` | Product-facing market-input readiness and ready-only observed-return evidence. Hashes, internal ids, raw prices, and FX values stay outside the UI DTO. |
...
| `/simulation` | Read-only Simulation Validation market-input readiness. Checks KODEX 200 and VOO independently for an exact end service date and 90 return steps, shows a bounded seven-date recheck history, and renders each ready input's complete 90-row historical KRW simple-return chart and table. | `src/app/simulation/page.tsx`, `getReadOnlySimulationInputReadiness`, the batch server-only period preflight, strict query selection, pure readiness and observed-return projections, and `SimulationInputReadinessView`. One bounded price/FX snapshot is read before the independent date/instrument preflights run in memory. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read. No browser refetch, provider, mutation, simulation execution, vector resolver, job, or persistence. Empty, whitespace-padded, invalid, and repeated `end` values are blocked before DB reads. | Pure model, batch-read, query-boundary, and route tests plus dedicated local/production smoke. The smoke accepts only `matrix_ready` or `unavailable`, requires seven bounded history rows, requires one complete observed-return series per ready input, scans for leaks, and compares DB row counts. | Market-data readiness and observed historical market-input returns only. The date rows are request-time rechecks, not persisted simulation runs. Incomplete inputs never expose partial return rows. The page does not expose raw prices/FX, fan paths, percentiles, initial capital, account selection, target weights, recommendation, or optimization. Exact-endpoint misses are not rolled back automatically; the user can explicitly inspect another displayed date through `?end=YYYY-MM-DD`. | Keep execution runtime blocked until tenant/vector authority and server-owned run parameters are established. |
