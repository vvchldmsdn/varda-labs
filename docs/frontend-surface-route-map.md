...
| `/investment-lab` | Read-only aggregate counterfactual comparing the observed all-account invested-position path with all-KODEX-200, all-VOO, an explicit KODEX/VOO fixed-mix path, a zero-return same-flow counterfactual, and an anchor-date observed equal-weight basket. The anchor basket starts from an exact stored portfolio/position source intersection, aggregates `(market,currency,ticker)` across accounts, and never backcasts today's holdings. Shows separately guarded Modified Dietz estimates, an ephemeral historical contribution-impact experiment, ETF look-through, and an ephemeral single-name linear shock experiment. | `src/app/investment-lab/page.tsx`, counterfactual and ETF X-ray server queries, read loaders/models, KODEX/VOO/fixed-mix/cash/anchor-basket path engines, contribution calculators, `investment-lab-etf-shock`, and narrow Client Components for local inputs. Reads event, account snapshot/cash/FX, bounded anchor position and price evidence, KODEX/VOO close, FX, ETF master, and latest ETF holding evidence. | Basic Auth via `src/proxy.ts`. | Server Components read the DB directly and pass sanitized read models to pure calculations. Independent ETF and portfolio-structure reads stream in separate Suspense boundaries. Anchor position evidence is loaded after the selected comparison axis is known, and broader bounded price evidence is skipped unless every anchor holding is identified. URL date, fixed-mix, and anchor selections are server-resolved. No browser refetch, provider, mutation, schema, job, recommendation, order, or persistence. | Build/test/lint and dedicated local/production route smoke cover fixed mix, zero-return baseline, anchor readiness/path, contribution, X-ray, and the default single-name shock while retaining leak and DB side-effect checks. | Aggregate `all` only. The zero-return same-flow path starts at the first observed invested-position value and applies the same boundary flows; it is not an actual cash ledger. The anchor basket allocates equally only once at the selected anchor, equal-splits later boundary flows, preserves per-market execution dates, and hides the whole path if any ticker, price, FX, or solvency evidence is incomplete. Contribution research is not the actionable account-specific Additional Contribution allocator. Exact flow-time TWR, actual cash-ledger reconstruction, scheduled rebalancing, hindsight optimization, transaction costs, prediction, and VaR remain deferred. | Keep tenant/auth-dependent execution blocked. Resolve stored tickerless/special-holding identity and complete bounded historical price evidence before claiming a production-ready whole-portfolio anchor-basket path. |
