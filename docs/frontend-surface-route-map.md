...
| `/history` | Read-only balance and portfolio history evidence. Supports account/lane URL filters, separate stored-amount trajectory charts, and exact named-account/date/source position drilldown. Stored versus display-only derived aggregates remain explicit and historical values are not recomputed under the current asset policy. | `src/app/history/page.tsx`, `getReadOnlyHistoryBalance`, `history-balance`, `history-trajectory`, `history-position-detail`, and `src/components/history/*`. Reads `account_balance_snapshots`, `daily_portfolio_snapshots`, and a bounded exact tuple from `daily_position_snapshots`; can derive display-only `all` portfolio rows when stored rows are absent. | Basic Auth via `src/proxy.ts`. | Server Component direct DB reads and server-rendered SVG/table. No browser refetch or write. Internal asset and legacy ids are removed before the display model. | Dedicated local and production smoke covers dashboard/history 401/200, account/lane chart/table states, one generated and one legacy-only position detail, stored/derived markers, no-interpolation disclosure, full-response ID/secret leakage, overflow ownership, and all three snapshot-table counts unchanged. | Charts connect only consecutive dates with the same source and row kind. Drilldown requires a stored named-account portfolio row and exact compatible position source; `all` derived rows, current-asset fallback, live prices, interpolation, normalization, performance reinterpretation, target/recommendation/order, and historical writes remain excluded. | Keep missing or mismatched stored evidence visible and choose the next product boundary separately. |
