...
| `/investment-lab` | Read-only aggregate counterfactual comparing the observed all-account invested-position path with all-KODEX-200, all-VOO, an explicit KODEX/VOO fixed-mix path, and a zero-return all-cash baseline under the same dated KRW buy/sell amounts. Shows separately guarded Modified Dietz estimates, an ephemeral historical contribution-impact experiment, ETF look-through, and an ephemeral single-name linear shock experiment. | `src/app/investment-lab/page.tsx`, counterfactual and ETF X-ray server queries, read loaders/models, KODEX/VOO/fixed-mix/cash path engines, contribution calculators, `investment-lab-etf-shock`, and narrow Client Components for local inputs. Reads event, account snapshot/cash/FX, KODEX/VOO close, FX, ETF master, and latest ETF holding evidence. | Basic Auth via `src/proxy.ts`. | Server Components read the DB directly and pass sanitized read models to pure calculations. Date/amount/scenario/shock interactions remain browser memory only. No browser refetch, provider, mutation, schema, job, recommendation, order, or persistence. | Build/test/lint and dedicated local/production route smoke cover fixed mix, cash baseline, contribution, X-ray, and the default single-name shock while retaining leak and DB side-effect checks. | Aggregate `all` only. The all-cash baseline starts at the first observed invested-position value, applies the same boundary flows, and assumes zero interest, fees, and tax; it is neither the current cash balance nor the account-specific Additional Contribution allocator. Contribution research is not the actionable account-specific Additional Contribution allocator. Shock results use the evaluated subset and observed ETF evidence without normalizing missing exposure; mixed evidence dates remain explicit. Exact flow-time TWR, total-return parity, scheduled rebalancing, hindsight optimization, transaction costs, prediction, and VaR remain deferred. | Keep tenant/auth-dependent execution blocked. The next read-only Investment Lab scenario should preserve the same boundary flows and explicitly avoid hindsight authority. |
