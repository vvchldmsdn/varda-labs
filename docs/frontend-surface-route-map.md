...
| `/simulation` | Read-only Simulation Validation and retrospective research surface for KODEX 200 and VOO. It keeps exact-date 90-step input readiness, bounded carry evidence, observed KRW-return charts, fixed single/fixed-mix stationary-bootstrap paths, regime-labeled hindsight research, and a separate walk-forward minimum-volatility comparison. | `src/app/simulation/page.tsx`, `getReadOnlySimulationInputReadiness`, the batch server-only period preflight, strict query selection, pure matrix/alignment/research modules, and split Simulation Server Components. One bounded price/FX snapshot is read before independent and combined preflights run in memory; all research projections reuse the combined matrix without another DB read. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read and pure in-memory calculations. No browser refetch, provider, mutation, vector resolver, job, or persistence. Empty, whitespace-padded, invalid, and repeated `end` values are blocked before DB reads. | Pure model and route tests cover the exact 90/91 invariant, cross-market carry alignment, no-look-ahead fold selection, within-fold drift, covariance regularization, unavailable-result isolation, leak scanning, and unchanged DB counts. Dedicated route smoke accepts only explicit ready/unavailable states. | All outputs are retrospective research, not account optimization, prediction, recommendation, or order authority. The walk-forward path uses only the paired KRW-return matrix: rolling 60-row training, the next 10-row test, three folds, long-only fully-invested weights, 10% diagonal covariance shrinkage, costless rebalance at fold boundaries, and no rebalance within a fold. A same-cadence 50:50 path is the neutral comparator. Missing joint evidence blocks only that research section. Raw prices/FX, current holdings, targets, Fount, KRX Gold, approved QQQ vector, tenant identity, and durable artifacts do not enter this path. Exact-endpoint misses are never rolled back automatically. | Keep tenant/account binding and actionable optimization blocked until authenticated owner and execution-input authority are established. Extend other hindsight objectives only with their own no-look-ahead policy. |
