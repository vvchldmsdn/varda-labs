...
| `/investment-lab` | Read-only aggregate counterfactual comparing the observed all-account invested-position path with an all-KODEX-200 path under the same dated KRW buy/sell amounts. Shows a guarded Modified Dietz estimate and VOO evidence readiness without a partial VOO path. | `src/app/investment-lab/page.tsx`, `getReadOnlyInvestmentLabCounterfactual`, read loader/model, return-evidence guard, return-estimate composer, pure Modified Dietz helper, VOO readiness validator, existing execution schedule, and deterministic path engine. Reads event, account snapshot/cash/FX, KODEX/VOO close, and FX projections in parallel. | Basic Auth via `src/proxy.ts`. | Server Component direct DB read and pure calculation. No browser refetch, provider, mutation, schema, job, or persistence. | Local production-mode smoke with production SELECT data: auth pass, KODEX return ready, VOO readiness 27/27 valuation close, 27/27 snapshot FX, 38/38 execution FX, leak scan pass, DB counts unchanged. Full production deployment smoke follows the commit. | Aggregate `all` plus KODEX 200 path only. Account-specific paths remain blocked by three unattributed events. Exact flow-time TWR, total-return parity, VOO path/performance, fixed vectors, scheduled rebalancing, optimization, and transaction costs remain deferred. | Review VOO readiness meaning, then implement the separate raw-close KRW path without widening to user vectors. |
