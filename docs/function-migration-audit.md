...
| `calcDiversification` | `portfolio_risk_read_adapter_migrated_ui_pending` | Descriptive diversification/risk analytics plus provider orchestration | `portfolio-risk-input*`, `portfolio-risk-statistics.ts`, `portfolio-risk-derived-metrics.ts`, `portfolio-risk.ts`, `portfolio-risk-read-model.ts`, and server-only DB adapter | Canonical input/math/read adapter is implemented and SELECT-only audited. Read-only UI remains pending; provider orchestration, composite scores, recommendation, beta/alpha/MDD, and legacy rounding are not ported. |
