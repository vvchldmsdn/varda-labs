...
- per-instrument gross growth-factor materialization (Phase 1B pure helper
  ...
  pending);
- normalized buy-and-hold portfolio-path semantics (Gate 0 approved against
  commit `652b9ea`; pure Scenario Vector Review Packet implemented but no
  numeric vector approved);
