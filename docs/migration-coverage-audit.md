...
| `GlobalMarketFactor` | `migrated` | 2,401 | 2025-06-01 to 2026-06-23 | `global_market_factors` | Imported by `scripts/import-base44-market-context.mjs`. Scalar factor keys/date/value/change fields are separated and `derived_metrics_json` is stored as JSONB. Phase 1E-E keeps this shared reference ownerless and reports legacy status/estimated evidence separately from current `is_preliminary`. |
...
| `MarketRegimeDaily` | `migrated` | 69 | 2026-05-20 to 2026-07-05 | `market_regime_daily` | Imported by `scripts/import-base44-market-context.mjs`. Preserves `drivers_json` as JSONB plus scores, labels, account/date, and nullable current account mapping. Phase 1E-E plans explicit ownership while preserving three legacy date/account duplicate groups as data-health evidence. |
