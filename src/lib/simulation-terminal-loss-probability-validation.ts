...
import type {
  ...
  ValidatedSimulationTerminalLossProbabilityInput,
...
export function validateSimulationTerminalLossProbabilityInput(
  ...
  const reasons = new Set<SimulationTerminalLossProbabilityBlockerReason>();
  ...
  return Object.freeze({
    validated: Object.freeze({
      normalizedNav:
        normalizedNav as ValidatedSimulationTerminalLossProbabilityInput["normalizedNav"],
      scenarioId,
      scenarioVersion,
...
function validateExpectedBinding(
  value: unknown,
  reasons: Set<SimulationTerminalLossProbabilityBlockerReason>,
) {
  if (
    !isRecord(value) ||
    !hasExactKeys(value, [
...
function blocked(
  reasons: ReadonlySet<SimulationTerminalLossProbabilityBlockerReason>,
): SimulationTerminalLossProbabilityValidationResult {
  return Object.freeze({
    validated: null,
...
function orderBlockers(
  reasons: ReadonlySet<SimulationTerminalLossProbabilityBlockerReason>,
): readonly SimulationTerminalLossProbabilityBlocker[] {
  return Object.freeze(
    SIMULATION_TERMINAL_LOSS_PROBABILITY_BLOCKER_ORDER.filter((reason) =>
      reasons.has(reason),
