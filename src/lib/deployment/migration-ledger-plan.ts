...
export type ReviewedMigrationPlan = {
  status: "ready";
  appliedCount: number;
  localCount: number;
...
export function planReviewedMigrations(input: {
  ...
  for (let index = 0; index < applied.length; index += 1) {
    ...
    if (
      ...
      const mismatch =
        actual.createdAt !== expected.createdAt ? "timestamp" : "hash";
      throw new Error(
        `Database migration ledger diverges (${mismatch}) at local migration ${expected.tag}.`,
      );
    }
  }
  ...
  for (const migration of pending) {
    ...
    if (
      ...
      throw new Error(
        `Pending migration is not allowlisted: ${migration.tag}.`,
      );
    }
  }
