...
export function MobileHoldingConnections({ graph }: { graph: Graph }) {
  ...
  return <div className={styles.mobile}>
    <p className={styles.intro}>{t("최근 동반 움직임", "Recent co-movement")}</p>
    ...
    <p className={styles.note}>{t("+1에 가까울수록 함께, −1에 가까울수록 반대로 움직였습니다. 미래 움직임이나 ETF 구성종목 겹침을 뜻하지 않습니다.", "Closer to +1 means they moved together; closer to −1 means they moved apart. This does not measure ETF holdings overlap or predict future moves.")}</p>
  </div>;
}
