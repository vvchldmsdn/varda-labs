  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold text-[var(--muted)]">
            <SimulationText ko={"별도 모형 · 부트스트랩과 합산하지 않음"} />{" "}</p>
          <h2
      ...
      <p className="mt-3 rounded-md border border-[var(--warning-soft)] bg-[var(--surface)] px-4 py-3 text-sm leading-6 text-[var(--warning)]">
        <SimulationText ko={"현재 저장 자료에는 각 지표의 정확한 공개 시각과 개정 이력이 없습니다. 같은 날 발표값은 다음 서비스 날짜부터 사용했지만 완전한 시점별 예측 검증은 아직 성립하지 않습니다. 이 결과는 회고적 연구이며 예측·추천·주문 근거가 아닙니다."} />{" "}</p>
      ...
      {result.status === "ready" ? (
        ...
        >
          <p className="font-semibold"><SimulationText ko={"이 모형만 아직 계산할 수 없습니다."} /></p>
          <p className="mt-1 text-sm leading-6 text-[var(--warning)]">
            <SimulationText ko={unavailableReasonLabel(result.reason)} /> {" "}<SimulationText ko={"기존 보유 비중 부트스트랩과 다른 검증 결과는 그대로 유지됩니다."} />{" "}</p>
          {result.remediation ? (
  ...
  return (
    ...
    >
      <header className="flex flex-col gap-2 border-b border-[var(--line)] px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold text-[var(--muted)]">
            <SimulationText ko={"현재 비중 · 최초 배분 후 리밸런싱 없음"} />{" "}</p>
          <h3 className="mt-1 text-lg font-semibold">
            <SimulationText ko={"요인·잔차 모형 ·"} />{" "}{result.assumptions.horizon}<SimulationText ko={"단계"} />{" "}</h3>
        </div>
        ...
        <table className="w-full min-w-[760px] border-collapse text-left text-sm">
          <thead className="text-xs text-[var(--muted)]">
            <tr>
              ...
              <th className="px-4 py-3 text-right font-semibold"><SimulationText ko={"미 10년물"} /></th>
              <th className="px-4 py-3 text-right font-semibold"><SimulationText ko={"장단기차"} /></th>
              <th className="px-4 py-3 text-right font-semibold"><SimulationText ko={"설명력 R²"} /></th>
            </tr>
          </thead>
          <tbody>
      ...
      <p className="border-t border-[var(--line)] px-4 py-3 text-xs leading-5 text-[var(--muted)]">
        <SimulationText ko={"결합 구간"} />{" "}{formatDate(result.source.firstAlignedServiceDate)}~
        {formatDate(result.source.lastAlignedServiceDate)} {" "}<SimulationText ko={"· 제외된 요인 공백"} />{" "}{" "}{result.source.factorGapRowCount}<SimulationText ko={"개 · 요인 공분산 축소 15% · 잔차 공분산 축소 25% · 자동 대체값 없음"} />{" "}</p>
    </article>
  );
}
