  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold text-[var(--muted)]">
            <SimulationText ko={"같은 정책 반복 점검 · 겹치는 기간"} />{" "}</p>
          <h2
            className="mt-1 text-lg font-semibold"
            id="walk-forward-stability-title"
          >
            <SimulationText ko={"워크포워드 기준일 안정성"} />{" "}</h2>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-[var(--muted)]">
            <SimulationText ko={"선택 기준일과 직전 6개 날짜에 같은 60/10/3 정책을 독립적으로 적용합니다. 날짜에 따라 결과가 크게 달라지는지 확인하는 진단이며, 가장 좋은 날짜나 설정을 고르는 기능이 아닙니다."} />{" "}</p>
        </div>
        <div className="text-left text-xs text-[var(--muted)] sm:text-right">
          <p className="font-semibold">
            {result.summary.readyEndpointCount}/
            {result.summary.endpointCount || result.policy.endpointCount}<SimulationText ko={"개 계산 가능"} />{" "}</p>
          <p className="mt-1"><SimulationText ko={"누락 날짜만 사용 불가로 유지"} /></p>
        </div>
      </div>

      {result.rows.length === 0 ? (
        <div className="mt-4 border-y border-[var(--warning-soft)] bg-[var(--surface)] px-4 py-4">
          <p className="font-semibold"><SimulationText ko={"안정성 이력을 계산하지 않았습니다."} /></p>
          <p className="mt-2 text-sm leading-6 text-[var(--warning)]">
            <SimulationText ko={reasonLabel(result.reason)} /> {" "}<SimulationText ko={"기존 단일 기준일 연구 결과는 그대로 유지합니다."} />{" "}</p>
        </div>
      ) : (
      ...
      <p className="mt-4 text-xs leading-5 text-[var(--muted)]">
        <SimulationText ko={"7개 검증창은 대부분의 학습·검증 수익률을 공유하므로 서로 독립된 7번의 실험이 아닙니다. 이 표를 이용해 학습기간, 10% 대각 축소, 재조정 주기를 다시 선택하거나 성과 순위를 만들지 않습니다."} />{" "}</p>
    </section>
  );
}
      ...
      <table className="w-full min-w-[980px] border-collapse text-left text-sm">
        <thead className="border-y border-[var(--line)] text-xs text-[var(--muted)]">
          <tr>
            ...
            <th className="px-3 py-3 text-right font-semibold"><SimulationText ko={"검증 수익률"} /></th>
            <th className="px-3 py-3 text-right font-semibold"><SimulationText ko={"연환산 변동성"} /></th>
            <th className="px-3 py-3 text-right font-semibold"><SimulationText ko={"50:50 변동성"} /></th>
            <th className="px-3 py-3 text-right font-semibold"><SimulationText ko={"변동성 차이"} /></th>
            <th className="px-3 py-3 text-right font-semibold"><SimulationText ko={"관측 MDD"} /></th>
            <th className="px-3 py-3 text-right font-semibold">
              <SimulationText ko={"KODEX 비중 3구간"} />{" "}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            ...
            >
              ...
              <td className="px-3 py-3">
                ...
                >
                  <SimulationText ko={row.status === "ready" ? "계산 가능" : "사용 불가"} />
                </p>
                {row.reason ? (
