...
export const simulationCalculationGuide = {
  ...
  steps: [
    ...
    {
      id: "paths", label: { ko: "500개 흐름", en: "500 paths" },
      title: { ko: "과거의 짧은 흐름을 다시 이어 붙여요", en: "Join short stretches of history in new orders" },
      body: {
        ko: "과거의 어느 날에서 시작해 다음 날들을 이어가다가, 다른 날로 다시 이동합니다. 평균 5개 구간씩 연결하는 이 과정을 반복해 500개의 가능한 경로를 만듭니다.",
        en: "Start at a sampled past date, continue through the following dates, then jump to another date. Repeat this with stretches averaging five intervals to create 500 possible paths.",
      },
      nodes: [
        { label: { ko: "과거의 짧은 묶음", en: "Short historical stretches" } },
        { label: { ko: "순서를 바꿔 연결", en: "Reconnect the stretches" } },
        { label: { ko: "500개 경로", en: "500 paths" }, detail: { ko: "63 또는 126단계", en: "63 or 126 steps" } },
      ],
      ...
      detail: {
        ko: "이 방식은 정상 블록 부트스트랩입니다. 매 단계 20% 확률로 새 시작일을 고르므로 묶음 길이는 매번 다르며 평균이 5입니다. 63·126은 수익률 관측 단계 수이며 달력 일수가 아닙니다. 같은 입력·설정이면 같은 경로를 재현합니다. 화면의 표본 경로는 12개만 보여주지만, 요약 수치는 500개 전체로 구합니다.",
        en: "This is a stationary bootstrap. Each step has a 20% chance of choosing a new starting date, so stretch lengths vary and average five. The 63 or 126 steps count return observations, not calendar days. Identical inputs and settings reproduce the same paths. Only 12 sample paths are displayed, but all 500 are used for the summary statistics.",
      },
    },
    {
      ...
      body: {
        ko: "차트의 가운데 선은 각 시점에서 500개 값의 중간값입니다. 손실로 끝난 비율은 마지막 값이 출발점보다 낮은 경로를 세고, 최대 하락폭은 각 경로가 중간 고점에서 얼마나 내려갔는지 봅니다.",
        en: "The middle line marks the middle of the 500 values at each step. The loss share counts paths ending below their start. Maximum drawdown measures each path’s largest drop from an earlier peak.",
      },
      nodes: [
        { label: { ko: "같은 500개 경로", en: "The same 500 paths" } },
      ...
      detail: {
        ko: "P10·P50·P90은 값을 작은 순서대로 놓았을 때 약 10%·50%·90% 위치의 경계입니다. 차트의 P10~P90은 각 시점에서 가운데 약 80% 경로의 범위입니다. MDD P90은 500개 경로의 최대 하락폭 중 약 90%가 그 이하인 경계로, 나머지 약 10%는 더 크게 하락했습니다. P50은 평균값이나 가장 가능성 높은 하나의 경로가 아닙니다.",
  ...
  notes: [
    { title: { ko: "80% 범위가 미래를 보장하나요?", en: "Does the 80% band guarantee the future?" }, body: { ko: "아니요. 과거 자료를 다시 조합한 500개 경로 안의 범위입니다. 새로운 사건이나 과거에 없던 움직임은 충분히 담지 못할 수 있습니다.", en: "No. It is a range within 500 paths assembled from past data. New events or movements absent from that history may not be represented well." } },
