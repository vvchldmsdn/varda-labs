    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          >
            <SimulationText ko={"국면별 고정 비중 3안 공통 경로"} />{" "}</h3>
          <p className="mt-1 max-w-4xl text-sm leading-6 text-[var(--muted)]">
            <SimulationText ko={"25:75, 50:50, 75:25를 같은 시장 상태·과거 후보·500개 추출 계획으로 계산합니다. 최초 배분 뒤에는 리밸런싱하지 않아 종목별 등락에 따른 비중 변화를 그대로 둡니다."} />{" "}</p>
        </div>
        <span className="w-fit rounded-md border border-[var(--line)] bg-[var(--surface)] px-3 py-1.5 text-xs font-semibold text-[var(--brand)]">
          <SimulationText ko={"비중 순서 · 성과 순위·추천 아님"} />{" "}</span>
      </div>
      ...
      {comparison.status === "ready" ? (
        ...
        >
          <p className="font-semibold"><SimulationText ko={"세 고정 비중을 함께 비교하지 않았습니다."} /></p>
          <p className="mt-2 text-sm leading-6 text-[var(--warning)]">
            <SimulationText ko={"공통 국면 실행 근거를 검증하지 못했습니다. 단일 종목이나 직접 입력한 비중의 독립 결과는 준비된 경우 그대로 유지합니다."} />{" "}</p>
        </div>
      )}
    </section>
  );
  ...
  return (
    ...
    >
      <p className="border-y border-[var(--line)] py-3 text-xs leading-5 text-[var(--muted)]">
        <SimulationText ko={"세 그래프는 하나의 세로축을 사용합니다. 카드는 KODEX 200 비중이 낮은 순서이며, 수익률이나 위험의 우열 순서가 아닙니다. stationary bootstrap 결과와 합산하거나 승패를 판정하지 않습니다."} />{" "}</p>
