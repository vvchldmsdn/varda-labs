    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold text-[var(--muted)]">
            <SimulationText ko={"현재 구성 고정 · 과거 약 1개월 관측 점검"} />{" "}</p>
          <h2
      ...
      {result.rows.length === 0 ? (
        <div className="mt-4 rounded-lg border border-[var(--warning-soft)] bg-[var(--surface)] px-4 py-3 text-sm leading-6 text-[var(--warning)]">
          <SimulationText ko={"현재 구성의 확률 경로가 먼저 계산되어야 과거 결과를 대조할 수 있습니다. 입력 점검에 표시된 종목과 이력 상태는 그대로 유지합니다."} />{" "}</div>
      ) : (
        <div className="mt-4 overflow-x-auto rounded-lg border border-[var(--line)] bg-[var(--surface)]">
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="border-b border-[var(--line)] text-xs text-[var(--muted)]">
              <tr>
                <th className="px-3 py-2 font-semibold"><SimulationText ko={"실제 종료일"} /></th>
                ...
                <th className="px-3 py-2 text-right font-semibold"><SimulationText ko={"예측 손실"} /></th>
                <th className="px-3 py-2 text-right font-semibold"><SimulationText ko={"실제 MDD"} /></th>
                <th className="px-3 py-2 text-right font-semibold"><SimulationText ko={"범위"} /></th>
              </tr>
            </thead>
            <tbody>
              {result.rows.map((row) => (
      ...
      <div className="mt-3 space-y-1 text-xs leading-5 text-[var(--muted)]">
        <p>
          <SimulationText ko={"이 결과는 현재 구성을 과거에 고정 적용한 사후 점검입니다. 당시 실제 보유량·매매 내역을 재현한 성과가 아니며, 비중 최적화나 주문 근거로 사용하지 않습니다."} />{" "}</p>
        <p>
          <SimulationText ko={"저장된 KIS 미조정 종가와 날짜별 환율을 사용하므로 배당·액면분할·병합을 반영한 총수익률과 다를 수 있습니다. 데이터가 부족한 행만 제외하고 계산 가능한 행은 유지합니다."} />{" "}</p>
      </div>
    </section>
  );
}
