  ...
  return (
    ...
    >
      <header className="border-b border-[var(--line)] pb-6">
        ...
        ><LabText value=" 과거 최고·최저 rolling 구간 " /></h2>
        <p className="mt-3 max-w-4xl text-sm leading-6 text-[var(--muted)]"><LabText value=" 저장된 완전 관측 구간 중 실제 포트폴리오의 현금흐름 조정 추정수익률이 가장 높고 낮았던 " />{model.policy.observationCount}<LabText value="개 관측일을 찾고, 같은 시작 평가액과 같은 매수·매도 흐름을 KODEX 200 및 VOO에 적용한 결과를 나란히 비교합니다. 사후 해석이며 예측·추천·최적화가 아닙니다. " /></p>
        <p className="mt-2 text-xs text-[var(--muted)]"><LabText value=" 후보 " />{model.candidateWindowCount}<LabText value="개 · 완전 관측" />{" "}
          {model.completeWindowCount}<LabText value="개 · 근거 불완전으로 제외" />{" "}
          {model.excludedWindowCount}<LabText value="개 " /></p>
      </header>
      ...
      {model.status === "ready" && model.worstWindow && model.bestWindow ? (
        <div className="grid gap-4 xl:grid-cols-2">
          <RollingWindowCard
            label="최저 구간"
            tone="negative"
            window={model.worstWindow}
          />
          <RollingWindowCard
            label="최고 구간"
            tone="positive"
            window={model.bestWindow}
          />
        </div>
      ) : (
        <div className="border-y border-[var(--warning-soft)] bg-[var(--surface)] px-4 py-4 text-sm text-[var(--warning)]"><LabText value=" 비교 가능한 완전 관측 rolling 구간이 2개 미만입니다. 일부 가격·환율· 거래 일정으로 결과를 보충하지 않고 이 섹션 전체를 표시하지 않습니다. " /></div>
      )}
    </section>
  );
}
    ...
    <article className="overflow-hidden rounded-[4px] border border-[var(--line)] bg-[var(--surface)]">
      <header className="border-b border-[var(--wash)] px-4 py-3">
        ...
        <p className="mt-1 text-xs text-[var(--muted)]">
          {window.observationCount}<LabText value="개 관측일 · 실제 흐름" />{" "}
          {window.actualFlowCount}<LabText value="건 " /></p>
      </header>
...
function MetricRow({
  ...
  return (
    <tr className="border-t border-[var(--wash)]">
      <td className="px-4 py-3 font-medium"><LabText value={label} /></td>
      <td className="px-3 py-3 text-right tabular-nums">
        {formatReturn(returnValue)}
      </td>
      <td className="px-3 py-3 text-right tabular-nums">
        <LabText value={difference === null ? "기준" : formatPercentagePoints(difference)} />
      </td>
      <td className="px-4 py-3 text-right tabular-nums">
        <LabText value={formatKrw(endValueKrw)} />
      </td>
    </tr>
  );
}
