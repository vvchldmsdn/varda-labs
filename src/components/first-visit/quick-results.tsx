...
export function QuickResults({ input }: { input: QuickInput }) {
  const result = analyzeQuickPortfolio(input);
  const [selected, setSelected] = useState("0");
  const row = result.rows.find(item => item.key === selected) ?? result.rows[0];
  return <section className={styles.results} aria-label="내 입력 자산 분석">
