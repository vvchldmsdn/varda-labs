...
export function PlanLibrary({ localAuthDisabled = false, hideEmpty = false }: { localAuthDisabled?: boolean; hideEmpty?: boolean }) {
  ...
  return <section className={styles.planLayout}><div className={styles.planMain}><p className={styles.eyebrow}>MY PLANS</p><h1>{draft?"다음 투자도, 여기서 이어가세요.":"내 투자계획"}</h1><p className={styles.privacy}>계획을 저장하면 자산과 목표는 그대로, 다음 투자금만 바꿔 다시 계산할 수 있어요.</p>
    ...
    {access==="unavailable"?<section className={styles.saveUnavailable} aria-label="저장 연결 안내"><h2>잠시 후 다시 시도해 주세요.</h2><p role="alert">{unavailableReason==="auth" || unavailableReason==="local" ? "지금은 로그인할 수 없어요. 계산 결과는 그대로 남아 있습니다." : "계획을 불러오지 못했어요. 계산 결과는 그대로 남아 있습니다."}</p><div className={styles.actions}><button className={styles.secondary} type="button" disabled={loading} onClick={()=>void load()}>{loading?"확인 중…":"다시 확인"}</button><Link className={styles.secondary} href="/try?mode=personal">{draft?"내 결과로 돌아가기":"투자금 계산하기"}</Link></div></section>:null}
    {access==="guest"?<><h2 className={styles.joinHeading}>입력은 한 번, 계획은 계속.</h2><p className={styles.privacy}>로그인하면 이 계획을 내 계정에 보관할 수 있어요.</p><div className={styles.actions}><Link className={styles.primary} prefetch={false} onClick={authIntent} href="/auth/sign-up">가입하고 저장 이어가기</Link><Link prefetch={false} onClick={authIntent} href="/auth/sign-in">기존 계정으로 로그인</Link><Link onClick={cancelAuthIntent} href="/try?mode=personal">가입하지 않고 결과로 돌아가기</Link></div></>:null}
    {access==="unlinked"?<><p className={styles.notice}>이메일·로그인 확인이 끝났습니다. 계획을 보관할 계정을 준비합니다. 계좌나 실제 자산은 만들지 않습니다.</p><label className={styles.confirmation}><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/>연결해야 할 기존 기록이 없으며 새 Cairn Labs 계정으로 시작합니다.</label><button className={styles.primary} disabled={!confirmed||pending} onClick={prepare}>{pending?"계정 준비 중…":"계정 준비하고 계획 확인"}</button><p><Link href="/auth/session?view=account">기존 기록 연결 확인</Link></p></>:null}
