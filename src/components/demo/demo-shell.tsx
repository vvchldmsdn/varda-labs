...
export function DemoShell({ view, account, children }: { view: DemoView; account: DemoAccount; children: ReactNode }) {
  return <main className={styles.page} data-demo-view={view}><EntryEvent event="demo_started" />
    <header className={styles.header}><Link href="/start" className={styles.logo}>VARDA</Link><span className={styles.badge}>샘플 포트폴리오</span><Link href="/try/analyze" className={styles.convert}>내 포트폴리오로 계속하기 ↗</Link></header>
    ...
    <section className={styles.stage}><div className={styles.heading}><div><p className={styles.eyebrow}>VARDA · 체험 모드</p><h1>{demoTitles[view]}</h1></div>{["home", "today", "structure", "contribution", "history"].includes(view) ? <nav className={styles.accounts} aria-label="샘플 계좌 선택">{([ ["all", "전체"], ["brokerage", "증권"], ["isa", "ISA"], ["irp", "IRP"] ] as const).map(([key, label]) => <Link prefetch={false} key={key} href={`/demo/${view}?account=${key}`} aria-current={account === key ? "page" : undefined}>{label}</Link>)}</nav> : null}</div>
      <p className={styles.disclaimer}>가상 금액·시세로 사용하는 실제 Varda 화면입니다. 실시간 정보나 추천 포트폴리오가 아닙니다.</p>
      {children}
    </section>
