...
export function PortfolioStructureView({data}:{data:PortfolioStructureViewData}) {
  ...
  const details=<>
    <Link className="varda-presentation-detail-trigger" href={`/portfolio/risk?scope=${encodeURIComponent(data.selectedScope.key)}${data.isDesignPreview?"&preview=design":""}`}><PortfolioText ko="상관·위험 보기" en="Correlation & risk" /></Link>
    <Link className="varda-presentation-detail-trigger" href={`/etfs?scope=${encodeURIComponent(data.selectedScope.key)}`}><PortfolioText ko="ETF 구성종목 보기" en="ETF holdings" /></Link>
