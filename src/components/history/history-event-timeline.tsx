...
export function HistoryEventTimeline({
  ...
  return (
    <div
      ...
      className="mt-3"
    >
      <p className="border border-[var(--brand-soft)] bg-[var(--wash)] px-3 py-2 text-sm leading-6 text-[var(--warning)]"><T ko="이벤트 일자는 저장된 이벤트 달력일이며 스냅샷 기준일·서비스 사이클과 별개입니다. 같은 기간에 함께 보이더라도 평가액 변동의 원인이나 성과 기여로 단정하지 않습니다." en="Event dates are recorded calendar dates, separate from snapshot dates and service cycles. Events shown in the same period do not establish the cause of value changes or performance contributions."/></p>
  ...
  return (
    <>
      {model.status === "partial" ? (
        <p className="mt-3 border border-[var(--brand-soft)] bg-[var(--wash)] px-3 py-2 text-sm text-[var(--warning)]"><T ko="일부 이벤트 근거가 불완전합니다. 확인 가능한 저장 행은 유지하며 금액을 복원하거나 정정 이벤트를 상계하지 않습니다." en="Some event data is incomplete. Verified rows remain visible; missing amounts are not reconstructed and correction events are not netted."/></p>
      ) : null}
      ...
      <div className="mt-3 overflow-x-auto">
        <table className="w-full min-w-[1320px] border-separate border-spacing-0 text-left text-sm">
          <thead className="text-xs text-[var(--muted)]">
            <tr>
              <TableHeader><T ko="이벤트 일자" en="Event date"/></TableHeader>
              <TableHeader><T ko="기록 시각" en="Recorded at"/></TableHeader>
              <TableHeader><T ko="유형 / 근거" en="Type / source"/></TableHeader>
              <TableHeader><T ko="종목" en="Holding"/></TableHeader>
              <TableHeader><T ko="자산 참조" en="Asset reference"/></TableHeader>
              <TableHeader align="right"><T ko="저장 금액" en="Recorded amount"/></TableHeader>
              <TableHeader align="right"><T ko="수량 변화" en="Quantity change"/></TableHeader>
              <TableHeader align="right"><T ko="저장 단가" en="Recorded unit price"/></TableHeader>
              <TableHeader align="right"><T ko="저장 환율" en="Recorded FX rate"/></TableHeader>
              <TableHeader><T ko="출처 / 규칙" en="Source / rule"/></TableHeader>
            </tr>
          </thead>
          <tbody>
      ...
      <p className="mt-3 text-xs leading-5 text-[var(--muted)]"><T ko="계정 미귀속 이벤트와 다른 계정 이벤트는 포함하지 않습니다. 현재 자산·현재 가격·fallback 환율로 과거 금액을 보완하지 않으며, 기간 수익률·TWR·성과 기여를 계산하지 않습니다." en="Unassigned events and events from other accounts are excluded. Historical amounts are not filled with current assets, prices or fallback FX rates. Period returns, TWR and performance attribution are not calculated here."/></p>
    </>
  );
}
...
function EventRow({ row }: { row: HistoryEventDisplayRow }) {
  return (
    ...
    >
      ...
      <TableCell>
        <span className="block font-semibold">{<T ko={eventTypeLabel(row)} en={translateHomeHistory(eventTypeLabel(row))}/>}</span>
        <span className="mt-1 block text-xs text-[var(--muted)]">
          {<T ko={evidenceLabel(row)} en={translateHomeHistory(evidenceLabel(row))}/>}
        </span>
      </TableCell>
      <TableCell>
        <span className="block font-semibold">{<T ko={row.ticker ?? "티커 없음"} en={translateHomeHistory(row.ticker ?? "티커 없음")}/>}</span>
        <span className="mt-1 block text-xs text-[var(--muted)]">
          {row.assetName}
      ...
      <TableCell>
        <span className="block">{<T ko={assetReferenceLabel(row)} en={translateHomeHistory(assetReferenceLabel(row))}/>}</span>
        {row.correctionStatus !== "none" ? (
          <span className="mt-1 block text-xs text-[var(--warning)]"><T ko="정정 참조 별도 보존" en="Correction references retained separately"/></span>
        ) : null}
      </TableCell>
      ...
      <TableCell>
        <span className="block">
          {<T ko={row.source ? historySourceLabel(row.source) : "출처 없음"} en={translateHomeHistory(row.source ? historySourceLabel(row.source) : "출처 없음")}/>}
        </span>
        <span className="mt-1 block text-xs text-[var(--muted)]">
          {<T ko={row.ruleVersion ?? "규칙 버전 없음"} en={translateHomeHistory(row.ruleVersion ?? "규칙 버전 없음")}/>}
        </span>
      </TableCell>
    </tr>
  );
