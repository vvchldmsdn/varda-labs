...
export const metadata: Metadata = {
  ...
  title: "내 자산을 이해하는 새로운 관점 | VARDA LABS",
  description: "자산이 왜 움직였는지, 어떤 구조인지, 다른 선택은 어땠을지. 샘플 포트폴리오로 Varda를 먼저 체험하거나 내 자산의 구성을 가입 없이 살펴보세요.",
  robots: { index: true, follow: true }, alternates: { canonical: "https://varda-labs.vercel.app/start" },
  openGraph: { title: "내 자산을 이해하는 새로운 관점 | VARDA", description: "먼저 써보세요. 마음에 들면 내 포트폴리오로 이어가세요.", images: [{ url: "/product-demo/poster-desktop.png", width: 1280, height: 800 }], type: "website" },
};
const chapters = [
...
export default function StartPage() {
  return <main className={styles.page}><EntryEvent />
    <nav className={styles.nav} aria-label="첫 방문 메뉴"><Link className={styles.brand} href="/start">VARDA</Link><div className={styles.navLinks}><Link href="/demo/home">서비스 체험</Link><Link href="/plans">내 기록</Link><Link href="/auth/sign-in" prefetch={false}>로그인</Link></div></nav>
    ...
    <footer className={styles.footer}><span>VARDA LABS</span><span>샘플은 가상 데이터입니다. 종목 추천·수익 보장·실제 주문을 제공하지 않습니다.</span></footer>
  </main>;
}
