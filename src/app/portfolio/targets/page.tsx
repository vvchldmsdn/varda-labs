      ...
      <div className="mx-auto w-full max-w-[1300px] space-y-4">
        <section className="rounded-lg border border-[var(--line)] bg-[var(--surface)] p-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-semibold tracking-normal"><ManagementText>{"목표비중"}</ManagementText></h1>
              <p className="mt-2 text-sm text-[var(--muted)]"><ManagementText>{"선택한 계좌 또는 자산그룹 안에서 새 투자금을 배분할 기준을 직접 정합니다."}</ManagementText></p>
            </div>
          ...
          <dl className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <SummaryCell label="선택 범위" value={selectedScope.label} localizeValue={false} />
        ...
        <section className="rounded-lg border border-[var(--line)] bg-[var(--surface)] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold">{selectedScope.label}{" "}<ManagementText>{"목표"}</ManagementText></h2>
              <p className="mt-1 text-sm text-[var(--muted)]"><ManagementText>{"저장하면 같은 범위의 이전 승인본은 수정되지 않고 종료 이력으로 남습니다."}</ManagementText></p>
            </div>
            <span className="rounded-md border border-[var(--line)] bg-white px-3 py-2 text-xs font-semibold text-[var(--muted)]"><ManagementText>{"계좌·종목별 목적지 보존"}</ManagementText></span>
          </div>
          ...
          {model.status === "ready" ? (
            ...
            <div className="mt-5 rounded-md border border-[var(--warning-soft)] bg-[var(--surface)] p-4 text-sm text-[var(--warning)]"><ManagementText>{"현재 범위의 보유종목 구성을 목표비중으로 만들 수 없습니다. 보유종목과 계좌 연결을 먼저 확인해 주세요."}</ManagementText></div>
          )}
        </section>
      </div>
    </main>
...
function SummaryCell({
  ...
  return (
    <div className="rounded-md border border-[var(--line)] bg-white p-4">
      ...
      <dd className="mt-2 text-lg font-semibold">{localizeValue ? <ManagementText>{value}</ManagementText> : value}</dd>
      {detail ? <p className="mt-1 text-xs text-[var(--muted)]"><ManagementText>{detail}</ManagementText></p> : null}
    </div>
  );
}
