import { deletePortfolioDraft, listPortfolioDrafts, savePortfolioDraft } from "@/db/queries/portfolio-drafts";
...
import { validateQuickPortfolio } from "@/lib/quick-portfolio";
...
export async function GET(request: Request) {
  try {
    ...
    return response({ drafts: await listPortfolioDrafts(tenant) });
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = validateQuickPortfolio(value.input);
    if (!parsed.ok) return response({ error: "invalid_input" }, 400);
    const tenant = await session();
    if (tenant instanceof Response) return tenant;
    const result = await savePortfolioDraft(tenant, value.id, parsed.input);
    if (result.status === "inactive") return response({ error: "app_user_not_active" }, 403);
    if (result.status === "conflict" || result.status === "limit") return response({ error: `draft_${result.status}` }, 409);
...
export async function DELETE(request: Request) {
  try {
    ...
    const tenant = await session();
    if (tenant instanceof Response) return tenant;
    if (!await deletePortfolioDraft(tenant, value.id)) return response({ error: "draft_not_found" }, 404);
