...
const EVIDENCE_FILE = join(
  tmpdir(),
  "varda-preview-database-preflight-v14.json",
);
...
async function run() {
  ...
  if (PHASE === "preflight") {
    ...
    const evidence = {
      evidenceVersion: "preview_database_build_preflight_v17",
  ...
  assert.equal(
    ...
    "preview_database_build_preflight_v17",
...
function assertPreflightCatalog(plan, state, localMigrations) {
  ...
  assert.deepEqual(
    plan.pendingTags,
    ["0035_worthless_thor"],
    "Only reviewed migration 0035 may be pending",
  );
  assert.equal(
    plan.latestAppliedTag,
    "0034_fine_ink",
    "Pending 0035 target must start from reviewed migration 0034",
  );
  ...
  assert.deepEqual(
    ...
    "Preview state and migration ledger disagree before migration 0035",
  );
