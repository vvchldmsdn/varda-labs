...
async function readCounts() {
  if (USE_REMOTE_DATABASE_EVIDENCE) {
    ...
    assert.equal(evidence.evidenceVersion, "preview_database_evidence_v8");
    ...
    assert.equal(
      evidence.targetPolicyCatalogStatus,
    ...
    assert.equal(
      evidence.portfolioScopeCatalogStatus,
      "reviewed_0024_present",
    );
    assert.equal(
      evidence.holdingOnboardingCatalogStatus,
      "reviewed_0025_present",
    );
    assert.equal(
